...
def convert_text_to_json_and_save(text_descriptions, text_file_paths, output_folder, save_path):
    """
    ...
    for  text_file_path in  text_file_paths:
        ...
        prompt = "Convert the below text file to json file with proper hierarchy and return only\
            ...
            miss any detail and write all the key and values in full detail."
...
def call_openai_and_save(image_paths, output_folder, text_file_paths, save_path, json_filename):
    ...
    for image_path in image_paths:
        ...
        base64_image_previous = encode_image(image_data)

        if(image_counter > 0):
            flag = True
            prev_image = previous_image_path
            buffer_prev = BytesIO()
            prev_image.save(buffer_prev, format='JPEG')  # You can choose format as needed (e.g., PNG)
            # Now, buffer.getvalue() contains the binary data you would get from image.read()
            image_data_previous = buffer.getvalue()
            base64_image_previous = encode_image(image_data_previous)

        previous_image_path = image_path

        prompt = "understand the next image and convert the full description of the image into text file with understanding of proper visual elements like \
                filled circle denotes that part is selected and hollow circle (white background in circle) is not selected and return only the \
        ...
        current_prompt =   " Above is the previous image from the previous prompt. (If there is no image before that means this is first prompt)" + prompt
        headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {api_key}"
        }   
        ...
        "messages": [
        {
            ...
            "content": [
            {
                "type": "image_url",
                "image_url": {
                "url": f"data:image/jpeg;base64,{base64_image_previous}"
                }
            },
            {
                "type": "text",
        ...
        text_descriptions.append(text_content)
        counter = counter + 1
        ...
        # save_at_path = os.path.join(save_path, filename)
        # with open(save_at_path, 'w') as text_file:
        #     text_file.write(text_content)
...
def combine_json_files(output_folder, pdf_path, save_path, json_filename):
    ...
    results_file_path = results_folder + 'ver_6.1_' + get_filename_without_extension(pdf_path) + '.json'
...
def call_end_to_end(pdf_path,save_path, json_filename):
    ...
    text_descriptions = []
    version_print()
    output_folder = 'output_ver_6_1/' + os.path.basename(pdf_path)[0:-4] + '/'
    image_paths = pdf_to_images(pdf_path)
    text_descriptions = call_openai_and_save(image_paths, output_folder, text_file_paths, save_path, json_filename)
    print("text file paths are : ", text_file_paths)
    print("text file description are : ", text_descriptions)

    convert_text_to_json_and_save(text_descriptions, text_file_paths, output_folder, save_path)
    combine_json_files(output_folder, pdf_path, save_path, json_filename)


# pdf_path = ''  # Replace with the path to your PDF file
# # file = get_filename_without_extension(pdf_path)
# # output_folder = 'output_ver_6_1/' + file + '/'
...
# output_folder = 'output_dpi/'
# text_file_paths = []
# # image_paths = pdf_to_images(pdf_path)
# # call_openai_and_save(image_paths, output_folder)
# # print(text_file_paths)
# text_file_paths = ['output_dpi/image_1.txt', 'output_dpi/image_2.txt', 'output_dpi/image_3.txt']
